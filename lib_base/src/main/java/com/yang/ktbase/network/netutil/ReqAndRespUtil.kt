 ...
 */
suspend fun <T> Response.executeHttp(block: suspend () -> BaseResponse<T>): BaseResponse<T> {
    runCatching {
        block.invoke()
 ...
 * 简易流处理，发送/订阅链式调用
 */
fun <T> Request.launchAndCollect(
 ...
 * 流处理：发送端
 */
fun Request.launchIn(showLoading: Boolean = false, requestBlock: suspend () -> Unit) {
    lifecycleScope.launch {
        flow {
            emit(requestBlock())
        }.onStart {
